...
const Contact = () => {
  ...
  return (
    <section id="contact" ref={scrollTab}>
      <h2 className="title contact-title" ref={(el) => el && divs.current.push(el)}>
        My Contacts
      </h2>
      <div className="description contact-description" ref={(el) => el && divs.current.push(el)}>
      ...
      <ul className="contact-list" ref={(el) => el && divs.current.push(el)}>
        {contactData.map((value, key) => (
          <li key={key} className="contact-listItem">
            <h3 className="contact-listItem-title">{value.title}</h3>
            <div className="contact-listItem-description">{value.value}</div>
          </li>
        ))}
      </ul>
    </section>
